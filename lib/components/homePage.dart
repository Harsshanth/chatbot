...
import 'package:chatbot/utils/album.dart';
...
class _MyHomePageState extends State<MyHomePage> {
        ...
        child: Column(
          children: <Widget>[
            Flexible(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) =>
                    buildChatBubble(messages[index]),
              ),
            ),
            SizedBox(
              height: 20,
            ...
            Container(
              child: ListTile(
                leading: IconButton(
                    onPressed: _hasSpeech != "yes" || speech.isListening
                        ? null
                        : startListening,
                    icon: Icon(Icons.mic)),
                title: Container(
                ...
                trailing: IconButton(
                    ...
                    onPressed: () async {
                      if (messageInsert.text.isEmpty) {
                        ...
                        setState(() {
                          messages = messages +
                              [Album(cnt: messageInsert.text, isBot: false)];
                          // button disable
                        });
                        try {
                          var album = await fetchAlbum();
                          setState(() {
                            messages = messages + [album];
                          });
                        } catch (e) {
                          setState(() {
                            messages = messages +
                                [Album(cnt: "Something happened try again")];
                          });
                        } finally {
                          // button enable
                        }
  ...
  Widget buildChatBubble(Album album) {
    return Container(
      padding: EdgeInsets.only(left: 20, right: 20),
      child: Row(
        mainAxisAlignment:
            album.isBot ? MainAxisAlignment.start : MainAxisAlignment.end,
        children: [
          album.isBot
              ? Container(
                  height: 60,
